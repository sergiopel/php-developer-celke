...
<body>
    ...
    if (!empty($dados['SendUpUsuario'])) {
        ...
        try {
            $query_up_usuario = "UPDATE usuarios 
                        SET nome=:nome, email=:email, senha=:senha, sits_usuario_id=:sits_usuario_id, niveis_acesso_id=:niveis_acesso_id, modified = NOW()
                        WHERE id=:id";
    ...
    try {
        ...
        $query_usuario = "SELECT id, nome, email, sits_usuario_id, niveis_acesso_id 
        FROM usuarios 
        WHERE id=:id 
        LIMIT 1";
    ...
    <form method="POST" action="">
        ...
        <select name="sits_usuario_id">
            ...
            <?php
            while ($row_sist_usuario = $result_sits_usuarios->fetch(PDO::FETCH_ASSOC)) {
                extract($row_sist_usuario);

                $select_sist_usuario = "";
                if (isset($dados['sits_usuario_id']) and ($dados['sits_usuario_id'] == $id)) {
                    $select_sist_usuario = "selected";
                } elseif (((!isset($dados['sits_usuario_id'])) and (isset($row_usuario['sits_usuario_id']))) and ($row_usuario['sits_usuario_id'] == $id)) {
                    $select_sist_usuario = "selected";
                }
